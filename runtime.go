...
func (o *PBool) Compare(a M) int {
	...
	return StrCmp(o.PType().Me().String(), a.Me().PType().Me().String())
}

// Uncommon but useful arithmetic on bools doesn't need to be efficient.
func (o *PBool) Add(b M) M { return MkInt(o.Int()).Me().Add(b) }
func (o *PBool) Sub(b M) M { return MkInt(o.Int()).Me().Sub(b) }
func (o *PBool) Mul(b M) M { return MkInt(o.Int()).Me().Mul(b) }
func (o *PBool) Div(b M) M { return MkInt(o.Int()).Me().Div(b) }

//func (o *PBool) IDiv(b M) M { return MkInt(o.Int()).Me().IDiv(b) }
func (o *PBool) Mod(b M) M { return MkInt(o.Int()).Me().Mod(b) }
...
func (o *PInt) BitAnd(a M) M             { return MkInt(o.N & a.Me().Int()) }
func (o *PInt) BitOr(a M) M              { return MkInt(o.N | a.Me().Int()) }
func (o *PInt) BitXor(a M) M             { return MkInt(o.N ^ a.Me().Int()) }
func (o *PInt) ShiftLeft(a M) M          { return MkInt(o.N << uint64(a.Me().Int())) }
func (o *PInt) ShiftRight(a M) M         { return MkInt(o.N >> uint64(a.Me().Int())) }
func (o *PInt) UnsignedShiftRight(a M) M { return MkInt(int64(uint64(o.N) >> uint64(a.Me().Int()))) }
...
func (o *PFloat) Add(a M) M     { return MkFloat(o.F + a.Me().Float()) }
func (o *PFloat) Sub(a M) M     { return MkFloat(o.F - a.Me().Float()) }
func (o *PFloat) Mul(a M) M     { return MkFloat(o.F * a.Me().Float()) }
func (o *PFloat) Div(a M) M     { return MkFloat(o.F / a.Me().Float()) }

//func (o *PFloat) IDiv(a M) M    { return MkInt(int64(o.F / a.Me().Float())) }
...
func (o *PStr) GetItemSlice(x, y, z M) M {
	...
	if y == None {
		...
		if j < 0 {
			panic(F("Second slicing index on PStr too small: %d", y.Me().Int()))
		}
	}
	if j > n {
...
func (o *PStr) Mod(a M) M {
	...
	return MkStr(F(o.S, a.Me().Contents()))
}
...
func (o *PStr) Mul(a M) M {
	...
	panic(F("Cannot multiply: str * %s", a.Me().PType()))
}
...
func (o *PStr) Contains(a M) bool {
	...
	panic(F("str cannot Contain non-str: %s", a.Me().PType()))
}
func (o *PStr) Add(a M) M { return MkStr(o.S + a.Me().String()) }
...
func (o *PByt) Mul(a M) M {
	...
	panic(F("Cannot multiply: byt * %s", a.Me().PType()))
}
...
func (o *PByt) Contains(a M) bool {
	...
	panic(F("Byt cannot Contain %s", a.Me().PType()))
}
...
func (o *PTuple) RType() string {
	...
	for i := 0; i < n; i++ {
		...
		buf.WriteString(o.PP[i].Me().RType())
	}
	buf.WriteString(")")
	return buf.String()
}
...
func (o *PTuple) Repr() string {
	...
	for i := 0; i < n; i++ {
		...
		buf.WriteString(o.PP[i].Me().Repr())
	}
	if n == 1 {
...
func (o *PDict) Repr() string {
	...
	for i := 0; i < n; i++ {
		...
		buf.WriteString(F("%q: %s", vec[i].Key, vec[i].Value.Me().Repr()))
	}
	buf.WriteString("}")
	return buf.String()
}
...
func (o *PDict) Compare(a M) int {
	...
	case *PDict:
		...
		b.mu.Unlock()
		return MkList(olist).Me().Compare(MkList(alist))
	}
	return StrCmp(o.PType().Me().String(), a.Me().PType().Me().String())
}
...
func (o *C_object) PickleFields(w *bytes.Buffer, v R.Value) {
	...
	for i := 0; i < nf; i++ {
		...
		if f.Anonymous {
			...
			v.Field(i).Interface().(M).Me().Pickle(w)
		}
	}
}
...
func (g *PGo) Dict() Scope {
	...
	case R.Map:
		...
		for _, k := range keys {
			...
			z[AdaptForReturn(k).Me().String()] = AdaptForReturn(v)
		}
	default:
...
func GoCast(want M, p M) M {
	typ := want.Me().Contents().(R.Type)
	return MkValue(AdaptForCall(p, typ))
}

func GoAppend(slice M, a M) M {
	return MkValue(R.Append(R.ValueOf(slice.Me().Contents()), R.ValueOf(a.Me().Contents())))
}
...
func adaptForCall2(v M, want R.Type) R.Value {
	...
	switch want.Kind() {
	case R.Uint8:
		return R.ValueOf(uint8(v.Me().Int()))
	case R.Uint16:
		return R.ValueOf(uint16(v.Me().Int()))
	case R.Uint32:
		return R.ValueOf(uint32(v.Me().Int()))
	case R.Uint64:
		return R.ValueOf(uint64(v.Me().Int()))
	case R.Int:
		return R.ValueOf(int(v.Me().Int()))
	case R.Int8:
		return R.ValueOf(int8(v.Me().Int()))
	case R.Int16:
		return R.ValueOf(int16(v.Me().Int()))
	case R.Int32:
		return R.ValueOf(int32(v.Me().Int()))
	case R.Int64:
		return R.ValueOf(v.Me().Int())
	case R.String:
		return R.ValueOf(v.Me().Str())
	case R.Func:
	...
	if want == typeInterfaceEmpty {
		...
		return R.ValueOf(v.Me().Contents())
	}
	...
	panic(F("Cannot AdaptForCall: %s [%s] %q [%s] TO %s [%s]", v, R.TypeOf(v), v.Me().Repr(), R.TypeOf(v.Me().Contents()), want, want.Kind()))
}
	...
	return R.MakeFunc(ft, func(aa []R.Value) (zz []R.Value) {
		...
		if err != nil {
			...
			default:
				zz = make([]R.Value, nout)
				for i := 0; i < nout; i++ {
					zz[i] = AdaptForCall(r.Me().GetItem(Mkint(i)), ft.Out(i))
				}
			}
		}
		if orig_nout > nout {
...
func CurrentStdout() io.Writer {
	if PtrSysStdout == nil || M(*PtrSysStdout).Me().Bool() == false {
		return os.Stdout
	}
	if w, ok := M(*PtrSysStdout).Me().Contents().(io.Writer); ok {
		return w
	}
	if pw, ok := M(*PtrSysStdout).Me().Contents().(PythonWriter); ok {
		return AdaptPythonWriter{PythonW: pw}
	}
	panic(F("CurrentStdout: not an io.Writer: %#v", M(*PtrSysStdout).Me().Contents()))
}

func CurrentStderr() io.Writer {
	if PtrSysStderr == nil || M(*PtrSysStderr).Me().Bool() == false {
		return os.Stderr
	}
	if w, ok := M(*PtrSysStderr).Me().Contents().(io.Writer); ok {
		return w
	}
	if pw, ok := M(*PtrSysStderr).Me().Contents().(PythonWriter); ok {
		return AdaptPythonWriter{PythonW: pw}
	}
	panic(F("CurrentStderr: not an io.Writer: %#v", M(*PtrSysStderr).Me().Contents()))
}
...
func CheckTyp(name string, obj M, typs ...M) {
	...
	panic(F("For %s, got object of type %s, wanted any of %v", name, obj.Me().PType().Me().String(), typs))
}
