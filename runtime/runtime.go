...
func (o *PSet) Compare(a M) int {
	switch b := a.X.(type) {
	case *PSet:
		o2 := native_sorted(MkList(o.List()), None, None, False)
		a2 := native_sorted(MkList(b.List()), None, None, False)
...
func native_mkbyt(a M) M {
	return MkByt(make([]byte, int(JInt(a))))
}

func native_range(a M) M {
	n := JInt(a)
	v := make([]M, n)
	for i := int64(0); i < n; i++ {
		v[i] = MkInt(i)
...
func newSorter(pp []M) *sorter {
	return &sorter{
		pp:  pp,
		cmp: func(a, b M) int { return JCompare(a, b) },
	}
}
...
func native_sorted(vec, cmp, key, reverse M) M {
	ps := JList(vec)
	if len(ps) == 0 {
		return MkList(nil)
	}
